...
Texture2D NormalSpecularMap : register(t9);
Texture2D SpecularMap : register(t10);
...
Texture2D ShadowMapLight1 : register(t7); // Texture holding the view of the scene from a light
Texture2D ShadowMapLight2 : register(t8);
...
float4 main(LightingPixelShaderInput input) : SV_Target
{
    ...
    float4 textureColour = DiffuseSpecularMap.Sample(TexSampler, input.uv) /*+ NormalSpecularMap.Sample(TexSampler,input.uv)*/;
    float3 diffuseMaterialColour = textureColour.rgb; // Diffuse material colour in texture RGB (base colour of model)
    float3 specularMaterialColour = textureColour.a; // Specular material colour in texture A (shininess of the surface)
    if (textureColour.a < gAlphaValue)
        discard;
    ...
    float3 finalColour = (diffuseLight * diffuseMaterialColour + specularLight * specularMaterialColour) * gDayCycle;
